...
namespace {
    ...
    std::string _getDataName(const Data& data) {
        auto* iv = dynamic_cast<IntValue*>(data.get());
        if (iv)  return std::to_string(iv->w);
    ...
    std::string _getSemanticsName(Semantics* sem) {
        auto* cs = dynamic_cast<ConstSemantics*>(sem);
        if (cs) return _getDataName(cs->w);
...
void ExternalSyGuSPBESolver::printSyGuSFile(const std::vector<Example> &example_list, const std::string &file_name) {
    ...
    for (auto& info: spec->info_list) {
        ...
        for (auto* nt: info->grammar->symbol_list) {
            ...
            for (auto* rule: nt->rule_list) {
                ...
                if (rule->param_list.empty()) current += _getSemanticsName(cr->semantics.get());
                else {
                    current += "(" + _getSemanticsName(cr->semantics.get());
    ...
    for (const auto& example: example_list) {
        ...
        for (const auto& data: io_example.first) {
            current += " " + _getDataName(data);
        }
        current += ") " + _getDataName(io_example.second) + "))\n";
        res += current;
    }
    res += "(check-synth)";
