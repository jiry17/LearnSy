...
ScorerCacheItem::ScorerCacheItem(TopDownContextGraph::Node *_node): node(_node), example_num(0) {
...
void ScorerCacheItem::initResList() {
    delete[] full_res;
...
ScorerCacheItem::~ScorerCacheItem() {
    delete[] full_res;
    for (auto* res: res_list) delete[] res;
}
std::unordered_map<std::string, std::vector<int> > ScorerCacheItem::getEqClass(const DataList &inp) {
...
void ScorerCacheItem::pushExample(const DataList &inp) {
...
void ScorerCacheItem::popExample() {
...
std::vector<RandomSemanticsScore*> ScorerCacheItem::getTerminateResult(const DataList &inp) {
...
void ScorerCacheItem::getFullResult(const DataList &inp, RandomSemanticsScore* res) {
    int n = example_num + 1;
...
ScorerCache::ScorerCache(TopDownContextGraph *_graph): graph(_graph) {
    for (int i = 0; i < graph->node_list.size(); ++i) {
        item_list.push_back(new ScorerCacheItem(&(graph->node_list[i])));
    }
}
void ScorerCache::pushExample(const DataList &inp) {
    for (auto& item: item_list) item->pushExample(inp);
}
void ScorerCache::popExample() {
    for (auto& item: item_list) item->popExample();
}
void ScorerCache::getFullResult(int node_id, const DataList &inp, RandomSemanticsScore *res) {
    item_list[node_id]->getFullResult(inp, res);
}
std::vector<RandomSemanticsScore*> ScorerCache::getTerminateResult(int node_id, const DataList &inp) {
...
OptimizedScorer::OptimizedScorer(Env *env, FlattenGrammar *fg, UnifiedEquivalenceModelLearner *_learner):
        Scorer(env, fg, _learner), cache_pool(new ScorerCache(fg->graph)) {
}
void OptimizedScorer::pushExample(const Example &inp) {
    Scorer::pushExample(inp); cache_pool->pushExample(fg->getFlattenInput(inp));
}
void OptimizedScorer::popExample() {
    Scorer::popExample(); cache_pool->popExample();
}
OptimizedScorer::~OptimizedScorer() {
    delete cache_pool;
}
...
public:
    ...
    DataList inp;
    UnifiedEquivalenceModel* last_model;
    ...
    OptimizedDifferentPairDPHolder(TopDownContextGraph* _graph, const std::vector<UnifiedEquivalenceModel*>& _model_list,
                                   const DataList& _inp, ScorerCache* _cache_pool): graph(_graph), model_list(_model_list),
...
RandomSemanticsScore OptimizedScorer::getScore(const PProgram &p, const Example &inp) {
