...
#include "istool/selector/samplesy/tree_encoder.h"
...
bool Z3VSASeedGenerator::checkEquivalent(Program* x, Program* y) {
    ...
    s.add(z3::mk_and(x_encode_res.cons_list));
    s.add(z3::mk_and(y_encode_res.cons_list));
...
ProgramList Z3VSASeedGenerator::getSeeds(int num, double time_limit) {
    ...
    if (count + 100 > res.size()) {
        ...
        z3::solver s(ext->ctx); s.add(z3::mk_and(cons_list));
        auto now_encode = ext->encodeZ3ExprForProgram(now.get(), ext::z3::z3Vector2EncodeList(param_list));
        s.add(z3::mk_and(now_encode.cons_list)); s.add(now_encode.res != encode_output);
        for (int _ = 0; _ < 100; ++_) {
